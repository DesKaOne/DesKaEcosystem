...
func TestValidateMessage(t *testing.T) {
	...
	signer, err := crypto.NewEd25519Signer(bytes.Repeat([]byte{0x42}, 32))
	if err != nil {
	...
	if err := ValidateMessage(m, testConsensusRules()); err != nil {
		t.Fatalf("validate: %v", err)
	}
	if err := VerifyMessageSignature(m, signer.PublicKey()); err != nil {
...
func TestValidateMessageRejectsWrongContext(t *testing.T) {
	...
	if err := ValidateMessage(m, ValidationRules{ProtocolVersion: 1, ChainID: 1001}); !errors.Is(err, ErrWrongProtocolVersion) {
		t.Fatalf("expected protocol version error, got %v", err)
	}
	m = testConsensusMessage()
	m.ChainID = 1002
	if err := ValidateMessage(m, ValidationRules{ProtocolVersion: 1, ChainID: 1001}); !errors.Is(err, ErrWrongChainID) {
...
func TestValidateMessageRejectsMalformedFields(t *testing.T) {
	...
	m := testConsensusMessage()
	if err := ValidateMessage(m, rules); !errors.Is(err, ErrMissingSignature) {
		t.Fatalf("expected missing signature error, got %v", err)
	}
	m.Signature = []byte{1}
	m.Sender = nil
	if err := ValidateMessage(m, rules); !errors.Is(err, ErrMissingSender) {
		t.Fatalf("expected missing sender error, got %v", err)
	}
	m.Sender = []byte{1}
	m.Type = 99
	if err := ValidateMessage(m, rules); !errors.Is(err, ErrInvalidMessageType) {
...
func TestValidateMessageRejectsOversizedPayload(t *testing.T) {
	...
	if err := ValidateMessage(m, ValidationRules{ProtocolVersion: 1, ChainID: 1001, MaxPayloadSize: 16}); !errors.Is(err, ErrMessageTooLarge) {
...
func TestConsensusSigningBytesAreDeterministicAndContextBound(t *testing.T) {
	...
	m.ChainID = 1002
	if bytes.Equal(a, m.SigningBytes()) {
		t.Fatal("signing bytes are not chain-bound")
	}
}
