...
func TestPoolAddDuplicateAndRemove(t *testing.T) {
    ...
    if err := p.Add(tx); err != nil { t.Fatal(err) }
    if err := p.Add(tx); err != ErrDuplicate { t.Fatalf("expected duplicate, got %v", err) }
    if p.Len() != 1 { t.Fatalf("expected length 1, got %d", p.Len()) }
...
func TestPoolCapacity(t *testing.T) {
    ...
    if err := p.Add(testTx(1)); err != nil { t.Fatal(err) }
    if err := p.Add(testTx(2)); err != ErrFull { t.Fatalf("expected full, got %v", err) }
}
