...
func TestCommitRuntimeFinalizedBlockConvergesAcrossTwoNodes(t *testing.T) {
	n1, ctx, candidate, _, validatorResolver, senderResolver, recipient := finalizedBlockFixture(t, storage.NewMemoryStore())
	n2, err := NewDevnet(storage.NewMemoryStore())
	if err != nil {
		t.Fatal(err)
	}

	// Both nodes start from the same canonical genesis/state and therefore must
	// converge on the same finalized block/state when given the same evidence.
	sender := types.Address([]byte("fixture-sender"))
	n2.State.Set(sender, state.Account{Balance: 60, Nonce: 0})
	...
	if err := n1.CommitFinalizedBlock(ctx, candidate, certificate, validators, power, validatorResolver, senderResolver); err != nil {
		t.Fatal(err)
	}
	if err := n2.CommitFinalizedBlock(ctx, candidate, certificate, validators, power, validatorResolver, senderResolver); err != nil {
		t.Fatal(err)
	}

	if n1.HeadHash != n2.HeadHash {
		t.Fatalf("node head hashes diverged: %x != %x", n1.HeadHash, n2.HeadHash)
	}
	if n1.State.Root() != n2.State.Root() {
		t.Fatalf("node state roots diverged: %x != %x", n1.State.Root(), n2.State.Root())
	}
	if got, ok := n1.State.Get(recipient); !ok || got.Balance != 20 {
		t.Fatalf("node 1 recipient balance = %d, want 20", got.Balance)
	}
	if got, ok := n2.State.Get(recipient); !ok || got.Balance != 20 {
		t.Fatalf("node 2 recipient balance = %d, want 20", got.Balance)
	}
}
