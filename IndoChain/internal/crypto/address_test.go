...
func TestBase58RoundTrip(t *testing.T) {
    ...
    for _, input := range cases {
        ...
        if !bytes.Equal(decoded, input) { t.Fatalf("round trip mismatch: %x != %x", decoded, input) }
    }
}
...
func TestAddressRoundTrip(t *testing.T) {
    ...
    if len(address) == 0 || address[:3] != "iND" { t.Fatalf("unexpected address: %s", address) }
    decoded, err := codec.Decode(address)
    if err != nil { t.Fatal(err) }
    if !bytes.Equal(decoded, payload) { t.Fatalf("payload mismatch") }
}
...
func TestAddressRejectsTampering(t *testing.T) {
    ...
    replacement := byte('1')
    if last == replacement { replacement = '2' }
    tampered := address[:len(address)-1] + string(replacement)
    if _, err := codec.Decode(tampered); err == nil { t.Fatal("tampered address unexpectedly accepted") }
}