...
func (c AddressCodec) Decode(address string) ([]byte, error) {
    if len(address) <= len(c.Prefix) || address[:len(c.Prefix)] != c.Prefix { return nil, ErrInvalidAddress }
    raw, err := Base58Decode(address[len(c.Prefix):])
    if err != nil || len(raw) < 5 { return nil, ErrInvalidAddress }
    body, checksum := raw[:len(raw)-4], raw[len(raw)-4:]
    if body[0] != c.Version { return nil, ErrInvalidAddress }
    expected := addressChecksum(body)
    if string(checksum) != string(expected) { return nil, ErrInvalidAddress }
