...
**CI failure and root cause:** IndoChain CI run `836` / push run `835` failed for `066f1bff2788fb5fc719a12082efa2bf7d311c3b` during `go test ./...`. The negative-path test treated `BlockProposal.Payload` as a slice although it is the fixed-size `types.Hash`, and the shared fixture passed `n.Config.BlockRules(nil)` without unpacking its `(rules, error)` result. The same failing test also exposed that `ValidatorRuntime.AcceptBlockProposal` was not recomputing the candidate payload; it only compared the proposal payload to itself. These were corrected in `c5fe45b09df64d35e922e3e4de893db6aeb7a094` and `ff63ae7009370f572fac50d85f38eb53ce03c446`.
