...
func TestNewFromEnvironmentPreservesEnabledProviderLifecycleOnRestart(t *testing.T) {
	statePath := filepath.Join(t.TempDir(), "provider-state", "state.json")
	t.Setenv("DIGIFLAZZ_USERNAME", "test-user")
	t.Setenv("DIGIFLAZZ_API_KEY", "test-key")
	t.Setenv("DESKAPROVIDER_PROVIDER_STATE_STORE_PATH", statePath)
	t.Setenv("DESKAPROVIDER_OPERATIONAL_STORE_PATH", filepath.Join(t.TempDir(), "operational", "snapshots.json"))
	t.Setenv("DESKAPROVIDER_TRANSACTION_STORE_PATH", filepath.Join(t.TempDir(), "transactions", "state.json"))
	...
	if !ok { t.Fatal("expected persisted digiflazz provider state after restart") }
	if !state.Enabled() { t.Fatal("expected enabled lifecycle to survive runtime restart") }
	if !state.Supports(operational.CapabilityPPOB) || !state.Supports(operational.CapabilityBalance) || !state.Supports(operational.CapabilityWebhook) {
		t.Fatalf("runtime restart must preserve refreshed capabilities: %#v", state.Capabilities)
	}
}
